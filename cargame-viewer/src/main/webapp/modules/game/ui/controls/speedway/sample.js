...
var game = {
	...
	'key_up' : function(e) {
		...
		if (code == 37 || code == 39) {
			steering_angle = 0.0;
			// This is called POWER STEERING, when the steering is released the
			// front wheels need to become straight very quickly
			steer_speed = 8.0;
		}
	},
...
function createWorld() {
	...
	debugDraw.SetSprite(document.getElementById("speedway").getContext("2d"));
	...
	createBox(world, game.screen_width / 2, 0.5, game.screen_width / 2 - 1,
			0.1, {
				'type' : b2Body.b2_staticBody,
				'restitution' : 0.5
			});
	createBox(world, game.screen_width - 1, game.screen_height / 2, 0.1,
...
// main entry point
$(function() {
	game.ctx = ctx = $('#speedway').get(0).getContext('2d');
	var canvas = $('#speedway');
...
function create_car() {
	...
	// front wheels
	left_wheel = createBox(world, car_pos.x - car_dim.x, car_pos.y + car_dim.y
			/ 2, wheel_dim.x, wheel_dim.y, {});
	right_wheel = createBox(world, car_pos.x + car_dim.x, car_pos.y + car_dim.y
			/ 2, wheel_dim.x, wheel_dim.y, {});

	// rear wheels
	left_rear_wheel = createBox(world, car_pos.x - car_dim.x, car_pos.y
			- car_dim.y / 2, wheel_dim.x, wheel_dim.y, {});
	right_rear_wheel = createBox(world, car_pos.x + car_dim.x, car_pos.y
			- car_dim.y / 2, wheel_dim.x, wheel_dim.y, {});
